...
V0.1 的产品行为仍以 Markdown 规范为权威；仓库同时包含一个离线 Python 实现层。它没有网站、服务端 API、后端、数据库、隐藏记忆或检索系统。
...
`plan` 输出 `EngineResult`；`prompt` 输出完整、与模型供应商无关的 `PromptPackage`；`validate-response` 对候选回答做确定性的结构和安全检查。为了向后兼容，`mingren-skill "输入"` 仍等同于 `plan`。

未来的供应商适配器可以把 `system_prompt`、`developer_prompt` 和 `user_prompt` 映射到对应消息格式，再把生成结果交给 `ResponseValidator`。当前版本不调用 OpenAI、Anthropic、DeepSeek 或其他 API，也不处理密钥、流式输出或网络请求；验证器不能保证事实正确。
